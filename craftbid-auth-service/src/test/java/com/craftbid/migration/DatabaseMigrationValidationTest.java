...
public class DatabaseMigrationValidationTest {
    ...
    @Test
    @DisplayName("Scenario 1: Fresh Database -> Flyway V1 -> Schema Verification")
    public void testScenario1_FreshDatabase_FlywayMigration() throws Exception {
        ...
        assertEquals(1, appliedMigrations, "Flyway should successfully execute V1 initial schema migration");
    ...
    @Test
    @DisplayName("Scenario 2: Existing Production Database -> Baseline Version 1 (No-op Safe Migration)")
    public void testScenario2_ExistingDatabase_BaselineAndMigrate() throws Exception {
        ...
        // Step A: Simulate pre-existing production database running full initial schema
        try (Connection conn = DriverManager.getConnection(dbUrl, USERNAME, PASSWORD);
             Statement stmt = conn.createStatement()) {
            ...
            stmt.executeUpdate("CREATE TABLE auctions (id BIGINT AUTO_INCREMENT PRIMARY KEY, craft_id BIGINT NOT NULL, seller_id BIGINT NOT NULL, starting_price DECIMAL(12, 2) NOT NULL, current_highest_bid DECIMAL(12, 2) DEFAULT NULL, reserve_price DECIMAL(12, 2) DEFAULT NULL, min_bid_increment DECIMAL(12, 2) NOT NULL DEFAULT 50.00, start_time DATETIME(6) NOT NULL, end_time DATETIME(6) NOT NULL, status VARCHAR(50) NOT NULL DEFAULT 'ACTIVE', winning_bidder_id BIGINT DEFAULT NULL, total_bids INT NOT NULL DEFAULT 0, max_participants INT NOT NULL DEFAULT 5, current_participants_count INT NOT NULL DEFAULT 0, interested_count INT NOT NULL DEFAULT 0, last_bid_time DATETIME(6) DEFAULT NULL, live_turn_active BOOLEAN NOT NULL DEFAULT FALSE, first_deposit_paid_at DATETIME(6) DEFAULT NULL, participation_deadline DATETIME(6) DEFAULT NULL, prep_deadline DATETIME(6) DEFAULT NULL, initial_wait_deadline DATETIME(6) DEFAULT NULL, turn_deadline DATETIME(6) DEFAULT NULL, admin_fee_amount DECIMAL(12, 2) DEFAULT NULL, artisan_payout_amount DECIMAL(12, 2) DEFAULT NULL, created_at DATETIME(6) NOT NULL, FOREIGN KEY (craft_id) REFERENCES crafts (id), FOREIGN KEY (seller_id) REFERENCES users (id))");
            stmt.executeUpdate("CREATE TABLE auction_participants (id BIGINT AUTO_INCREMENT PRIMARY KEY, auction_id BIGINT NOT NULL, user_id BIGINT NOT NULL, base_price_paid DECIMAL(12, 2) NOT NULL, total_amount_paid DECIMAL(12, 2) NOT NULL, status VARCHAR(50) NOT NULL DEFAULT 'JOINED', refund_amount DECIMAL(12, 2) DEFAULT 0.00, cancellation_fee DECIMAL(12, 2) DEFAULT 0.00, cancellation_refund_amount DECIMAL(12, 2) DEFAULT 0.00, cancellation_status VARCHAR(50) DEFAULT NULL, cancelled_at DATETIME(6) DEFAULT NULL, cancellation_requested_at DATETIME(6) DEFAULT NULL, joined_at DATETIME(6) NOT NULL, CONSTRAINT uk_auction_user_participant UNIQUE (auction_id, user_id), FOREIGN KEY (auction_id) REFERENCES auctions (id), FOREIGN KEY (user_id) REFERENCES users (id))");
        ...
        assertEquals(0, applied, "Flyway should baseline at V1 without executing redundant migrations");

        // Step C: Verify all columns remain present and untouched
        try (Connection conn = DriverManager.getConnection(dbUrl, USERNAME, PASSWORD);
             Statement stmt = conn.createStatement()) {
            Set<String> participantCols = getColumns(stmt, "auction_participants");
            assertTrue(participantCols.contains("cancellation_fee"));
